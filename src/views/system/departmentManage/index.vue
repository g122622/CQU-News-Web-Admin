...
import UserDrawer from "@/views/system/departmentManage/components/UserDrawer.vue";
...
const columns: ColumnProps<User.ResUserList>[] = [
    ...
    { prop: "qq", label: "QQ" },
    {
        ...
        label: "职位",
        render: scope => {
            return <el-tag>{scope.row.role === 0 ? "指导老师" : scope.row.role === 1 ? "部长" : "部员"}</el-tag>;
        }
    },
    {
        prop: "state",
        ...
        render: scope => {
            return (
                <el-tag type={scope.row.state != 2 ? "success" : "danger"}>
                    {scope.row.state === 0 ? "实习" : scope.row.state === 1 ? "在职" : "离职"}
                </el-tag>
            );
        }
    },
